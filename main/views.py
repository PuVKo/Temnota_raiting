...
def index(request):
    ...
    if request.method == 'POST':
        if request.POST.get('type') == 'season':
            ...
            # Обрабатываем таблицу table
            for new in table:
                merged_table.append(new)

            # Обрабатываем таблицу table2 и добавляем игроков, которых нет в table
            for old in table2:
                found_in_merged_table = False
                for existing in merged_table:
                    if old.nick == existing.nick:
                        existing.donwin += old.donwin
                        existing.donloose += old.donloose
                        existing.sherifwin += old.sherifwin
                        existing.sherifloose += old.sherifloose
                        existing.mirwin += old.mirwin
                        existing.mirloose += old.mirloose
                        existing.mafwin += old.mafwin
                        existing.mafloose += old.mafloose
                        existing.nowinner += old.nowinner
                        existing.dop += old.dop
                        existing.compens += old.compens
                        existing.straf += old.straf
                        existing.twomaf += old.twomaf
                        existing.threemaf += old.threemaf
                        existing.threered += old.threered
                        existing.total_win += old.total_win
                        existing.total_loose += old.total_loose
                        existing.total_game += old.total_game
                        existing.losses += old.losses
                        found_in_merged_table = True
                        break

                if not found_in_merged_table:
                    merged_table.append(old)

            # Обрабатываем таблицу table3 и добавляем игроков, которых нет в merged_table
            for old3 in table3:
                found_in_merged_table = False
                for existing in merged_table:
                    if old3.nick == existing.nick:
                        existing.donwin += old3.donwin
                        existing.donloose += old3.donloose
                        existing.sherifwin += old3.sherifwin
                        existing.sherifloose += old3.sherifloose
                        existing.mirwin += old3.mirwin
                        existing.mirloose += old3.mirloose
                        existing.mafwin += old3.mafwin
                        existing.mafloose += old3.mafloose
                        existing.nowinner += old3.nowinner
                        existing.dop += old3.dop
                        existing.compens += old3.compens
                        existing.straf += old3.straf
                        existing.twomaf += old3.twomaf
                        existing.threemaf += old3.threemaf
                        existing.threered += old3.threered
                        existing.total_win += old3.total_win
                        existing.total_loose += old3.total_loose
                        existing.total_game += old3.total_game
                        existing.losses += old3.losses
                        found_in_merged_table = True
                        break

                if not found_in_merged_table:
                    merged_table.append(old3)

            # Теперь merged_table содержит объединенные данные из всех таблиц
            table = merged_table
